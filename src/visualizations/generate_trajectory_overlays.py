"""
Generate trajectory overlay figures comparing RL policy vs CORALL baseline across Imazu cases.

Usage
-----
  # All 22 cases (stacked + grid figures):
  python -m src.visualizations.generate_trajectory_overlays \\
      ...
      --rl_step 850000 \\
      --output_dir "1_Visualizations/trajectory_overlays/compare_all22_step850k"

Key arguments
-------------
  --cases INT [INT ...]    Case numbers to generate overlays for (default: 1 10 18)
  --base_dir PATH          Directory containing corall_baseline_case* folders
  --rl_dir PATH            Directory containing eval_cp*_case* folders
  --rl_step INT            Optional: filter to eval_cp<step>_case* (e.g. 850000)
  --output_dir PATH        Output directory for all figures
  --rl_only_grid           Generate RL-only grid (no baseline required)
  --grid_output_name STR   Filename for --rl_only_grid output

Generated figures
-----------------
  case<N>_full_trajectory.png          Per-case RL vs baseline trajectory
  case<N>_encounter_detail.png         Encounter zoom
  case<N>_threat_profile.png           CPA threat panel
  trajectory_comparison_stacked.png    All cases stacked (RL + baseline)
  trajectory_comparison_grid.png       All cases grid (RL + baseline)
  trajectory_rl_only_stacked.png       All cases stacked (RL only)
"""
...
from src.path_setup import ensure_paths
ensure_paths()

from src.visualizations.episode_overlay_tools import (
    animate_ship,
...
# Shared trajectory color semantics across generated figures.
COLOR_BASELINE = "#1f77b4"
COLOR_RL = "#ff7f0e"
COLOR_TARGET = "#b9c0ca"
COLOR_START = "gold"
COLOR_GOAL = "darkgreen"
...
def plot_stacked_rl_only_trajectory_overlay(
    ...
    outer_gs = gridspec.GridSpec(
        n_rows, n_cols,
        left=0.03, right=0.995, top=0.88, bottom=0.055,
        hspace=0.62, wspace=0.28,
        figure=fig,
    )

    ownship_color = COLOR_RL
    target_color = COLOR_TARGET
    start_color = COLOR_START
    goal_color = COLOR_GOAL
    ...
    fig.savefig(save_path, dpi=400, format='png', bbox_inches='tight')
    plt.close(fig)
    return Path(save_path)


def plot_comparison_trajectory_grid(
    case_data: list[tuple[dict, dict, int]],
    save_path: Path,
) -> Path:
    """
    Grid layout comparing RL and rule-based (CORALL baseline) ownship trajectories.

    Matches the visual format of plot_stacked_rl_only_trajectory_overlay but overlays
    both ownship tracks: RL in orange, baseline in blue.
    """
    import matplotlib.gridspec as gridspec
    ...
    ownship_rl_color = COLOR_RL
    ownship_bl_color = COLOR_BASELINE
    target_color = COLOR_TARGET
    start_color = COLOR_START
    goal_color = COLOR_GOAL
    ship_icon_interval_s = 60.0
    ...
    for row, (bl_hist, rl_hist, case_num) in enumerate(case_data):
        r = row // n_cols
        c = row % n_cols
        ax = fig.add_subplot(outer_gs[r, c])
        _, Xr, _, _, _, _ = to_numpy_history(rl_hist)
        _, Xb, _, _, _, _ = to_numpy_history(bl_hist)
        n_agents = Xr.shape[1]

        # Target trajectories (grey, from RL history for alignment).
        for j in range(1, n_agents):
            lbl = "Target ship(s)" if (row == 0 and j == 1) else None
            ax.plot(
                Xr[:, j, 0],
        ...
        # Baseline (rule-based) ownship track.
        xb0, yb0 = Xb[:, 0, 0], Xb[:, 0, 1]
        lbl_bl = "CORALL baseline" if row == 0 else None
        ax.plot(xb0, yb0, "-", color=ownship_bl_color, linewidth=1.8, label=lbl_bl, zorder=2)

        # RL ownship track.
        xr0, yr0 = Xr[:, 0, 0], Xr[:, 0, 1]
        lbl_rl = "RL policy" if row == 0 else None
        ax.plot(xr0, yr0, "-", color=ownship_rl_color, linewidth=1.8, label=lbl_rl, zorder=3)

        # Ship icons along RL ownship for visual alignment with the RL-only stacked figure.
        max_time = max(1.0, float(Xr.shape[0] - 1))
        icon_times = np.arange(ship_icon_interval_s, max_time, ship_icon_interval_s)
        for t_icon in icon_times:
            idx_icon = int(np.clip(round(t_icon), 0, Xr.shape[0] - 1))
            ...
            animate_ship(
                ...
                beam_m * ship_scale,
                cpa=0.0,
                color=ownship_rl_color,
                ax=ax,
            )

        # Start / goal markers (from RL track).
        lbl_start = "Ownship start" if row == 0 else None
        lbl_goal = "Goal" if row == 0 else None
        ax.scatter(xr0[0], yr0[0], s=40, color=start_color, label=lbl_start,
                   zorder=6, edgecolors="black", linewidth=0.5)
        ...
        case_x = (quad_bbox.x0 + quad_bbox.x1) * 0.5
        case_y = quad_bbox.y1 + 0.01
        fig.text(case_x, case_y, f"Case {case_num}",
                 ha="center", va="bottom", fontsize=22, fontweight="bold")
        fig.text(case_x, case_y - 0.020, f"({n_agents} ships)",
                 ha="center", va="bottom", fontsize=11, fontweight="normal")
        ...
        # Bounds from all ownship + target trajectories combined.
        all_x = np.concatenate([Xr[:, j, 0] for j in range(n_agents)] + [xb0])
        all_y = np.concatenate([Xr[:, j, 1] for j in range(n_agents)] + [yb0])
    ...
    handles = [
        Line2D([0], [0], color=target_color, lw=1.5, alpha=0.8, label="Target ship(s)"),
        Line2D([0], [0], color=ownship_bl_color, lw=1.8, label="CORALL baseline"),
        Line2D([0], [0], color=ownship_rl_color, lw=1.8, label="RL policy"),
        Line2D([0], [0], marker='o', color='none', markerfacecolor=start_color,
               markeredgecolor='black', markersize=6, label="Ownship start"),
        Line2D([0], [0], marker='X', color='none', markerfacecolor=goal_color,
               markeredgecolor=goal_color, markersize=7, label="Goal"),
    ...
    fig.suptitle(
        "RL vs CORALL Baseline Ownship Trajectories Across All 22 Imazu Cases",
        y=0.995, fontsize=31, fontweight="bold"
    )
...
def main():
    ...
    if len(stacked_data) >= 2:
        ...
        grid_path = args.output_dir / "trajectory_comparison_grid.png"
        print(f"\nGenerating comparison grid figure ({len(stacked_data)} cases)...")
        plot_comparison_trajectory_grid(stacked_data, grid_path)
