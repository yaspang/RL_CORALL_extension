...
# Shared trajectory color semantics.
...
def plot_ownship_threat_profile(
    baseline_hist: Dict,
    rl_hist: Dict,
    save_path: str | Path,
    own_idx: int = 0,
    ...
    """
    Simplified OWNSHIP-ONLY CPA analysis (for clean comparison to CORALL outputs).
    Shows AGGREGATE threat profile: max risk, min range, min DCPA across all targets
    Baseline vs RL without per-target clutter.
    ...
    # Compute aggregate threat metrics (across all targets)
    # Max risk across all targets
    risk_b_agg = series_max_risk(pair_riskb, own_idx=own_idx)
    risk_r_agg = series_max_risk(pair_riskr, own_idx=own_idx)
    
    # Min DCPA across all targets (filtered by encounter distance)
...
def plot_full_trajectory_overlay(
    ...
    ax.plot(xb0, yb0, "-", color=COLOR_BASELINE, linewidth=1.8,
            label="CORALL baseline", zorder=3)
    ax.plot(xr0, yr0, "-", color=COLOR_RL, linewidth=1.8,
            label="RL policy", zorder=3)

    # Start marker
    ax.scatter(xb0[0], yb0[0], s=60, color=COLOR_START, label="Ownship start",
               zorder=6, edgecolors="black", linewidth=0.6)

    # Goal marker (simple X at ownship end)
    goal_x = (xb0[-1] + xr0[-1]) / 2.0
    goal_y = (yb0[-1] + yr0[-1]) / 2.0
    ax.scatter(goal_x, goal_y, marker="X", s=80, color=COLOR_GOAL,
               zorder=6, label="Goal")
    ...
    for t_icon in icon_times:
        ...
        animate_ship(
            ...
            LOA * ship_scale, BEAM * ship_scale,
            cpa=0.0, color=COLOR_BASELINE, ax=ax,
        )
...
def plot_stacked_trajectory_overlay(
    ...
    for row, (baseline_hist, rl_hist, case_num) in enumerate(case_data):
        ...
        ax.plot(xb0, yb0, "-", color=COLOR_BASELINE, linewidth=1.5, label=lbl_bl, zorder=3)
        ax.plot(xr0, yr0, "-", color=COLOR_RL, linewidth=1.5, label=lbl_rl, zorder=3)

        # Start / goal markers
        lbl_start = "Ownship start" if row == 0 else None
        lbl_goal = "Goal" if row == 0 else None
        ax.scatter(xb0[0], yb0[0], s=40, color=COLOR_START, label=lbl_start,
                   zorder=6, edgecolors="black", linewidth=0.5)
        goal_x = (xb0[-1] + xr0[-1]) / 2.0
        goal_y = (yb0[-1] + yr0[-1]) / 2.0
        ax.scatter(goal_x, goal_y, marker="X", s=50, color=COLOR_GOAL,
        ...
        for t_icon in icon_times:
            ...
            animate_ship(
                ...
                LOA * ship_scale, BEAM * ship_scale, cpa=0.0, color=COLOR_BASELINE, ax=ax,
            )
            if rl_active:
                animate_ship(
                    float(Xr[ir_idx, 0, 0]), float(Xr[ir_idx, 0, 1]), float(Xr[ir_idx, 0, 2]),
                    LOA * ship_scale, BEAM * ship_scale, cpa=0.0, color=COLOR_RL, ax=ax,
                )
...
def save_episode_history(history: Dict, output_path: str | Path) -> Path: 
    ...
    np.savez_compressed(
        ...
        final_waypoint_x_nmi = np.asarray([history.get("final_waypoint_x_nmi", None)], dtype=object),
        final_waypoint_y_nmi = np.asarray([history.get("final_waypoint_y_nmi", None)], dtype=object),
    )
...
def load_episode_history(path: str | Path) -> Dict:
    ...
    history = {
        ...
        "final_waypoint_x_nmi": float(data["final_waypoint_x_nmi"][0]) if "final_waypoint_x_nmi" in data and data["final_waypoint_x_nmi"][0] is not None else None,
        "final_waypoint_y_nmi": float(data["final_waypoint_y_nmi"][0]) if "final_waypoint_y_nmi" in data and data["final_waypoint_y_nmi"][0] is not None else None,
    }
...
def make_overlay_figure_set(
    ...
    cpa_panel = plot_ownship_threat_profile(
        baseline_history,
        trained_history,
        output_dir / "ownship_threat_profile.png",
        own_idx=own_idx,
        dcpa_ylim=dcpa_ylim,
