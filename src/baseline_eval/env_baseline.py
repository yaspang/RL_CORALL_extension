...
"""
CORALL rule-based baseline environment for ownship evaluation.

Runs CORALL's reactive avoidance guidance on ownship (ship_0) and propagates
target ships along scripted straight-line trajectories. Produces the same
episode metrics as env_RL_ppo.py for direct comparison with RL evaluation results.

Not run directly — instantiated by eval_baseline_with_hist.py:
    python -m src.baseline_eval.eval_baseline_with_hist --case 6 --episodes 100
"""
...
class ObsNorm: 
    ...
    # Velocity bounds in m/s (matches env_RL_ppo.py)
    vel_max: float = 15.0
...
class CORALLComparisonEnv(ParallelEnv):
    """
    CORALL reactive-avoidance baseline for a single Imazu case.

    Ownship (ship_0) is controlled by CORALL's waypoint planner and reactive
    avoidance logic. Target ships follow scripted straight-line trajectories via
    obstacle_sim(). Actions passed to step() are ignored.

    Produces ownship-centric episode metrics identical to env_RL_ppo.py:
    path length, min DCPA, risk exposure, min separation, success,
    ...
    def update_pairwise_geometry_cache(self) -> None:
        ...
        - For ownship (agent 0): velocity-based CPA (cpa_calculations_0speed)
        - For obstacles: projection-based CPA (scripted trajectories)
        """
    ...
    def compute_dones(self):
        ...
        # Track minimum DCPA: active encounter only (< 3 nmi), clipped at 5 nmi, artifacts < 10m removed.
        # 3 nmi threshold matches compare_case_metrics.py so baseline and RL metrics are comparable.
        dcpa_vals = self.pair_dcpa[k].copy()
        dist_vals = self.pair_dist[k].copy()
        
        # Filter: exclude self, non-finite values, and pairs outside active encounter range (< 3 nmi)
