"""
Procedural encounter environment — fully geometry-based.
...
Imazu case numbers from third_party/CORALL/src/imazu_cases.py are used ONLY as structural templates 
(for agent count / action-observation space sizing). All geometry is replaced with the backwards-computed 
positions above, so training is fully case-independent.
...
class RandomEncounterEnv(gym.Wrapper):
    ...
    # ---- Stochastic mixed curriculum (active) ----
    ...
    def update_step(self, step: int):
        """Call from training loop to advance curriculum implementation."""
