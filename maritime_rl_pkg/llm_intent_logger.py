...
Logged fields (one row per active TS encounter per decision interval):
  ...
  colreg_rule, llm_response, parsed_kdir
...
def _extract_kdir(response: str) -> int:
    """
...
class LLMIntentLogger:
    ...
    def _build_llm_prompt(self, active_feats: List[Dict]) -> str:
        ...
        return f"""You are a ship navigation officer. Make a COLREGs-compliant decision from the encounter geometry below.
            ...
            try:
                ...
                if provider is None:
                    ...
                    if _is_error_response(llm_response):
                        ...
                        parsed_kdir = _extract_kdir(llm_response)
            except Exception as exc:
